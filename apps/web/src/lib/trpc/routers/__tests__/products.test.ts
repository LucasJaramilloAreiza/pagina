...
describe("products.delete", () => {
  ...
  it("removes the product link from existing order items before deleting", async () => {
    const product = await caller.create({ name: "Linked", price: 100, in_stock: 1 });
    ...
    await expect(caller.delete({ id: product.id })).resolves.toEqual({ success: true });

    const [item] = await db.select().from(orderItems).where(eq(orderItems.order_id, createdOrder.id));
