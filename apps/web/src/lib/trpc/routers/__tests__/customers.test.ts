...
import { paymentMethods, products } from "@/lib/db/schema";
...
describe("customers.delete", () => {
  ...
  it("removes the customer link from existing orders before deleting", async () => {
    const customer = await caller.create({ name: "Linked", email: "linked@t.com" });
    ...
    const [paymentMethod] = await db.insert(paymentMethods).values({ name: "Efectivo" }).returning();

    const { ordersRouter } = await import("../orders");
    const orderCaller = createCallerFactory(ordersRouter)({ user: makeUser("user-1") });

    const createdOrder = await orderCaller.create({
      customerId: customer.id,
      paymentMethodId: paymentMethod.id,
      products: [{ id: product.id, quantity: 1, price: 1000 }],
      total: 1000,
    });

    await expect(caller.delete({ id: customer.id })).resolves.toEqual({ success: true });

    const order = await orderCaller.get({ id: createdOrder.id });
